...
read_csv("~/Desktop/LandDemand/FOFA2050CountryData_Crop-production 7.csv") #Import FOFA 2050 data

LandDemand <- FOFA2050CountryData_Crop_production_7
LandDemand$Domain <- NULL
...
LandDemand <- spread(LandDemand, Indicator, Value)
LandDemand$`Climate shifter` <- NULL
LandDemand$`Technology shifter` <- NULL 
LandDemand$`Harvested area` <- LandDemand$`Harvested area` * 1000
LandDemand$Production <- LandDemand$`Harvested area` * LandDemand$`Crop yield`
LandDemand <- spread(LandDemand, Element, Production)
colnames(LandDemand) <- c("Item", "CountryName", "Year","Arable land" ,"Crop yield","Cropping intensity", "Harvested Area", "Prod_I", "Prod_R")
...
LandDemand_Projected <- LandDemand_2050[,c(1,2,8,9)]
LandDemand_Projected$proj <- 0
LandDemand_Projected <- LandDemand_Projected %>% #aggregate any duplicate rainfed or irrigated values so each crop-country value is not duplicated. 
...
LandDemand_Baseline <-LandDemand_Baseline  %>% #aggregate any duplicate rainfed or irrigated values so each crop-country value is not duplicated. 
  group_by(Item, CountryName) %>%
  summarise_each(funs(sum))
LandDemand_Baseline$Harv_R <-  LandDemand_Baseline$Prod_R/LandDemand_Baseline$Yld_R 
LandDemand_Baseline$Harv_I <-  LandDemand_Baseline$Prod_I/LandDemand_Baseline$Yld_I 
...
LandDemand_High <-LandDemand_High  %>% #aggregate any duplicate rainfed or irrigated values so each crop-country value is not duplicated. 
  group_by(Item, CountryName) %>%
  summarise_each(funs(sum))
LandDemand_High$High <- LandDemand_High$Harv_I + LandDemand_High$Harv_R #total land demand for high scenario
...
LUC_baseline <- merge(LandDemand_Baseline[, c(1,2,7,8),],LandDemand_2012[,c(1,2,9:12)])
...
LUC_baseline$LUC <- ifelse(LUC_baseline$Base_R == 0,(LUC_baseline$Base_I/LUC_baseline$CropIntensity_I) - LUC_baseline$Arable_I ,ifelse(LUC_baseline$Base_I == 0, (LUC_baseline$Base_R/LUC_baseline$CropIntensity_R) -  LUC_baseline$Arable_R, ((LUC_baseline$Base_R/LUC_baseline$CropIntensity_R) + (LUC_baseline$Base_I/LUC_baseline$CropIntensity_I)) - (LUC_baseline$Arable_I + LUC_baseline$Arable_R)))
...
LUC_projected  <- merge(LandDemand_2050[, c(1,2, 5:10)],LandDemand_2012[,c(1,2,11,12)]) #Combine "Item", "Countryname", 2050 production (rainfed and irrigated), 2050 crop intensity (irrigatd and rainfed), 2012 Arable land (irrigated and rainfed)
...
```{r pressure, echo=FALSE}
# Calculate LUC by crop and country for high  =  land demand/crop intensity(I,R) - 2012 arable land(I,R) ; assume 2050 cropping intensity; high land demand = harvested area for 2012. Need to calculate new cropping intensity. Yields are higher than FOFA 2050 yield so cropping intensity must be higher. 
...
#Calc new yield for High-yield scenario
...
#Calc new crop intensity for high-yld scenario. New crop intensity = (high yield * FOFA crop intensity)/FOFA yield
...
#Convert land demand to arable land with cropping intensity,cannot use original arable land from FOFA data 
LUC_high$LUC <- ifelse(LUC_high$Harv_R == 0,(LUC_high$Harv_I/LUC_high$new_CI_I) - LUC_high$Arable_I ,ifelse(LUC_high$Harv_I == 0, (LUC_high$Harv_R/LUC_high$new_CI_R) -  LUC_high$Arable_R, ((LUC_high$Harv_R/LUC_high$new_CI_R) + (LUC_high$Harv_I/LUC_high$new_CI_I)) - (LUC_high$Arable_I + LUC_high$Arable_R)))
```
